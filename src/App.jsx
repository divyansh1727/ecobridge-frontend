...
const [requestId, setRequestId] = useState(null);
  ...
  { step: "home", selectedRole: null, comparisonResults: null },
  "",
  ""
);
    }, 2500);
  ...
  { step, selectedRole, comparisonResults },
  "",
  ""
);
    }
  ...
  useEffect(() => {
    const handlePopState = (event) => {
      if (event.state) {
        ...
        setComparisonResults(event.state.comparisonResults);
    };
  }
...
/>
      ...
      <div className="pt-20">
        <AnimatePresence mode="wait">
          ...
          {step === "wasteForm" && (
  <WasteForm
    onSubmitWaste={(results,wasteType,location) => {
      setComparisonResults({results,wasteType,location});
      setStep("comparison");
    }}
    onBack={() => setStep("role")}
  />
...
{step === "requestStatus" && requestId && (
  ...
  />
          ...
          {/*comparsion block*/}
          {step === "comparison" && comparisonResults && (
  <ComparisonTable
    results={comparisonResults.results}
    wasteType={comparisonResults.wasteType}
    location={comparisonResults.location}
    onBack={(id) => {
      if (id) {
        setRequestId(id);
        setStep("requestStatus");
      } else {
        setStep("wasteForm");
      }
    }}
  />
          ...
          {step === "generatorDashboard" && (
  <GeneratorDashboard onBack={() => setStep("home")} />
)}
          {step === "login" && (
  <motion.div
    key="login"
